...
// UpdateCenterScheduler is a ticker for update-center honesty.
//
...
func (s *UpdateCenterScheduler) runSyncLocked(dbw *store.DB) {
	_ = dbw
	// Known limitation : log-only no-op for product purposes.
	...
	// updateAvailable=false + field); deploy/rollback/SSE are honest
	// 501 known limitations. This scheduler must not
	// invent "update available", fake lastCheckedAt, or fake task completion.
	// TODO (not wired): wire actual update-center polling only when a
