...
// without importing proxy (avoids scheduler↔proxy cycles).
var (
...
func (s *ChannelRecoveryScheduler) runSweepLocked(dbw *store.DB) {
	...
	// hook. When unset, SQL LIMIT 50 approximation remains.
...
func (s *ChannelRecoveryScheduler) loadActiveCandidates(dbw *store.DB) []recoveryCandidate {
	// Prefer coordinator-active IDs when app has wired the optional provider.
	// Presence of the provider (not the slice contents) selects this path so an
	// empty active-lease set does not fall back to the SQL scan.
	if fn := getActiveChannelIDsProvider(); fn != nil {
		return s.loadActiveCandidatesFromIDs(dbw, fn())
	}

	// Known limitation fallback : SQL approximation of "active" channels
	...
	// semantics. Honest known limitation only when the optional hook is unset.
	var candidates []recoveryCandidate
...
func (s *ChannelRecoveryScheduler) probeCandidate(dbw *store.DB, candidate recoveryCandidate, nowMs int64) {
	...
	// Prefer shared model probe executor when registered.
