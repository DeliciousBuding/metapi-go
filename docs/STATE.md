...
| UI i18n verification | **present (v0.9.0 rewrite)** | key-based i18n (i18next): en + zh-CN locales each **1475** keys; vitest i18n-keys gate keeps both key sets identical — bidirectional 0 missing; shared `InterfaceControls` on login/header (language + appearance + light/dark) + browser-language auto-follow (localStorage → navigator) + `document.documentElement.lang` sync via `toBcp47`; sidebar nav fully key-based (sidebar.groups/items) |
| Settings v2 | **implemented and browser-verified** | unified dirty/save/error guards; semantic daily/interval/window/custom schedules; additive one-click migration; legacy cron compatibility; 1440px and 390px checks pass |
