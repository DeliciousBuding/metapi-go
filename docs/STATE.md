...
| Theme accent presets | **chart-synced (v0.9.0 rewrite)** | 5-axis theme (preset/font/radius/scale/content-layout) + 10 presets; charts read the `--chart-1…5` OKLCH palette as CSS `var()` directly (recharts SVG; the legacy VChart canvas + `useChartColors()` JS extraction was removed in #718); header ThemeCustomizer panel (preset swatches / font Auto-Sans-Serif / radius / scale + per-axis & global reset); **all presets default to sans-serif** (serif is an explicit font-axis choice, no preset inlines it) |
