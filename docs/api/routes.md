...
A `refreshModels` pass is one upstream round-trip per active account, so on a large fleet it outlives any sane HTTP client timeout. The work therefore runs on a context detached from the request: if the browser or a reverse proxy hangs up, the refresh and the rebuild still run to completion instead of dying with the connection, and the closing rebuild always happens — including when every account refresh failed, because routes are composed from the availability already stored. Clients should still set a generous timeout so the reported counts are the real ones.
