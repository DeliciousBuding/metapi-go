...
**Tip**: `37d6a70` · tag **v0.8.45** · unreleased: parity core + P0-555 obs + P0-585 HTTP e2e + #557 procedure + CI unblock + dual-dialect Context helpers + engineering optimization wave + N2-N7/G1 productization batch + all-api-hub borrow Wave A (A1 余额历史 / B1 需关注看板 / D1 per-task 通知+4 渠道) + Wave B (E1 随机窗口 / F1 导入预览 / C1 调度统一运行历史)  
...
5. Product borrow backlog（决策输入，未立项）: [`../analysis/competitive/all-api-hub-product-borrow-2026-07-31.md`](../analysis/competitive/all-api-hub-product-borrow-2026-07-31.md) — **Wave A 已发（A1/B1/D1）+ Wave B 已发（E1/F1/C1）**；余 P1-P3 候选（A2 模型成本分布 / G1 批量验证 / I1 标签 / H1 风险横幅 / K1 模型映射 / J1 快照 PNG）需拍板再动，不静默实现。
