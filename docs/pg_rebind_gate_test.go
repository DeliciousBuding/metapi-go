...
var (
	...
	dbCallRe = regexp.MustCompile(
		`\b(\w+)\.(Exec|Get|Select|Query|QueryRow|QueryRowx|Queryx|ExecContext|GetContext|SelectContext|QueryContext)\(\s*` +
	...
	for _, dir := range dirs {
		...
		err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
			...
			for _, line := range strings.Split(content, "\n") {
				for _, m := range dbCallRe.FindAllStringSubmatch(line, -1) {
					if m[3] == "" {
