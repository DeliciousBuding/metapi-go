...
>
> **2026-08-18 知识卫生收口**：原 P1 #1（checkin/oauth 死复选框）、#2（排序清除）、#4（Stat 卡升级）三项经核实已交付，从下表移除。证据：`checkin-page.tsx`/`oauth-page.tsx` 均为 `enableRowSelection: false`；`data-table/core/column-header.tsx` 已有「Default order」菜单调 `clearSorting()`；`dashboard/components/stat-card.tsx` 已有 IconBadge + 三档 tone + 明细子格（注释标注 audit upgrade）。原编号 #3/#5/#6 重排为 #1/#2/#3。P2 #5（校验失败 focus-first-invalid）：导入向导显式实现 `markInvalidAndFocusFirst` + `aria-invalid` + per-field clear（2026-08-18 补 4 个回归用例，`import-wizard-dialog.test.tsx` 共 9 例）；经核实 `account-form-dialog.tsx` 等其余全部 RHF 表单由 `ui/form.tsx` 的 `FormValidationFocus` 全局覆盖（初始 baseline 即存在），补 account/site 对话框 2 个行为用例验证，全部收口。
...
| 5   | 已交付          | 导入向导校验失败聚焦首错字段              | `import-wizard-dialog.tsx`、`ui/form.tsx`           | 全部收口：导入向导显式实现 `markInvalidAndFocusFirst`（9 用例）；其余 RHF 表单由 `ui/form.tsx` `FormValidationFocus` 全局覆盖（校验失败后滚动并聚焦首个 `aria-invalid` 控件），account/site 对话框行为用例验证（2026-08-18） |
...
- 交互审计：1 P0 / 8 P1；已修复倍率行内编辑安全网、页码钳位、导入脏确认和 404/错误边界；focus-first-invalid 全部收口（导入向导显式实现 + `ui/form.tsx` `FormValidationFocus` 全局覆盖经行为用例验证）。其余内容保留为审计观察。
