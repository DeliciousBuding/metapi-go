...
| S3a | TOTP 2FA + step-up 二次认证 | admin auth = Bearer token + IP CIDR 白名单，无登录态 | **待拍板（决策包见 §9）**——推荐：**保持现状**（Bearer + IP 白名单对内部单管理员足够；登录/会话体系是售卖场景（S5 非目标）前置） |
...
| S7 | 异步/批量图片 + 视频生成任务 | images/videos 转发面 | **待拍板（决策包见 §8）**——推荐：**关闭**（批量任务编排面向外部售卖用户（S5 非目标）；内部使用单发转发已覆盖） |
...
| B4 批量媒体任务（S7） | M | **待拍板**——决策包 §8：推荐关闭（售卖场景=非目标 S5，内部单发转发已覆盖） |
| B5 登录/2FA 体系（S3a） | M | **待拍板**——决策包 §9：推荐保持现状（Bearer + IP 白名单对内部单管理员足够） |
...
## 8. B4 拍板决策包（2026-08-01）

**问题**：是否实现异步/批量图片 + 视频生成任务编排（任务队列 + 状态查询 + 批量提交）。

**现状**：metapi-go 有 images/videos 转发面（单发同步转发）。

**选项**：
| 选项 | 成本 | 收益 |
|:-----|:-----|:-----|
| A. **关闭（推荐）** | 0 | 批量媒体任务的消费方是外部售卖用户（S5 非目标）；内部使用单发转发已覆盖；任务编排（队列/状态/回调）是整套产品面 |
| B. 实现 | M 级（任务表 + 队列 + 轮询状态 + 批量 API + 前端任务页） | 仅未来向外部售卖生成类 API 时有价值 |

**推荐**：A 关闭。与 B3/S5 同一逻辑（售卖闭环=非目标）。未来外部售卖再开。

## 9. B5 拍板决策包（2026-08-01）

**问题**：是否建立登录/会话体系 + TOTP 2FA + step-up 二次认证。

**现状**：admin auth = Bearer AUTH_TOKEN（每次请求直验）+ IP CIDR 白名单（N1）。无用户/密码登录态，无会话。

**分叉本质**：2FA/step-up 需要「登录态」作为承载点（登录取会话 → 敏感操作 step-up）。当前模型是「每请求带 token」——没有可承载 2FA 的中间状态。

**选项**：
| 选项 | 成本 | 收益 |
|:-----|:-----|:-----|
| A. **保持现状（推荐）** | 0 | 内部工具 + 单管理员：Bearer + IP 白名单（N1）已构成双重防护；无多用户场景（S5 非目标） |
| B. 实现 | M 级（会话表 + 登录页改造 + TOTP 绑定/验证 + step-up 中间件 + 前端 2FA 流程） | 仅当外部用户/多管理员接入时有价值 |

**推荐**：A 保持现状。若未来开放多管理员/外部访问再开（届时登录体系是 B5 的前置，2FA 随后）。

**拍板方式**：与 B3/B4 同批一句话拍板（「B4 关闭」「B5 保持」）；本文档即决策记录。
