...
| FormControl label wiring (form fields)   | **gated, partial debt**      | `web/scripts/check-form-control.mjs` (chained into `bun run lint`): 140 `<FormControl>` sites classified, **0 unclassified** (the gate fails on a shape it cannot read rather than skipping it). 131 reach a DOM node carrying the injected `id`; 9 call sites across 5 composite components are registered exceptions — see §7 item 4 and #1300. |
...
4. **Composite form controls have no accessible name** — five components rendered inside `<FormControl>` do not forward the `id` / `aria-describedby` / `aria-invalid` it clones onto its child, so their `<FormLabel htmlFor>` points at an id that exists nowhere: `EndpointsEditor` (empirically confirmed — `queryByLabelText('API endpoints')` returns `null` while every sibling field in the same form resolves), `ScheduleEditor` (4 call sites), `ModelPolicyEditor`, `SiteScopePicker` and `CredentialRefPicker` (2 call sites). The failure is silent: no type error, no runtime error, and the CI `a11y` scan does not reach inside these dialogs. A composite has no single labelable element, so the remedy is `role="group"` + `aria-labelledby` on the group rather than forwarding one id onto an arbitrary inner input. Gated by `web/scripts/check-form-control.mjs`, where each is a registered exception that **must be deleted once fixed** (a stale entry fails the gate, so the registry cannot rot). Tracked in #1300; the forwarding reference implementation is `web/src/features/sites/components/custom-headers-field.tsx`.
