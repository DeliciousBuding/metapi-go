...
- **Never call `bun install` directly in a workflow step or the Dockerfile.** Use `scripts/bun-install.sh`: it retries *only* transient registry/CDN tarball-integrity faults and clears bun's install cache between attempts, while a frozen-lockfile mismatch, a 404 or a postinstall failure still fails on the first attempt. `scripts/bun_install_wiring_test.go` enforces the wiring in both places. Background: four different packages failed this way across three jobs in one day, and the Dockerfile's cache mount pointed at `/root/.bun/install-cache` while bun uses `/root/.bun/install/cache`, so it never cached anything. "Reruns green" proves a fault is transient; it does not prove it needs no fix.
