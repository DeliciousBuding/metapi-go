...
## 2026-09-03 — v0.17.1 切版（备份表集注册表化 · 设置水合不再毁值 · **CI 竞态门复活**）

- **版本判断**：patch-first。v0.17.0 打 tag 之后又合入两个运维可见修复（#1172 备份导出缺 5 张表、#1173 一条坏设置行清空已配置值）与一个 CI 完整性修复（#1175），按 `git-workflow.md` §6.1「每波合入 master 且含用户可见变更 → 立即 bump 最后一位」走 v0.17.1；不构成新里程碑，故不动中间位。
