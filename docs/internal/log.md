...
- **复验脚本自身的两处「验证了自己没验证的东西」（同日修）。脚本属私有层，这里记的是它暴露出的两条关于**本仓**的事实**：① `make build` **不重建前端**（Makefile 无前端目标），于是脚本把磁盘上任意龄期的 gitignored `web/dist` 烘进二进制 ⇒ 此前几次「合并后复验」的 F5 视觉步骤验的是**旧 UI**；现打印 dist 溯源（mtime / 主包名 / `index.html` sha256 前 16 位）并在源文件比 dist 新时 WARNING 点名，另加 `REBUILD_WEB=1`（实测 ~15s、峰值 used ~1.2GB）与 `RELEASE_BINARY=<path>`（改为验证已发布产物）两个模式。**龄期只能用 mtime 判不能用内容哈希判**：同一份源码在本机两次构建得到 `index.bfd4430f47.js` / `index.669a83d73f.js`，CI 发布产物是 `index.7340ee69cd.js`，rsbuild 哈希跨构建主机不可复现。② `GET /api/about` **需要 admin bearer**，脚本此前裸调、把 `{"error":"Missing Authorization header"}` 打印出来就往下走 ⇒ 唯一一行用来证明「当前跑的是哪个构建」的输出从来没证明过任何事；现改为带 token 请求并与被测物对账（`RELEASE_BINARY` 比 `version`，本地构建比 `commit` vs `git rev-parse HEAD`），不一致即 `fail=1`。**这与 #1175 是同一类故障：一个不检查自己前提的检查。**
- **负结果（别再查）**：`actionlint` **不是** #1175 那一类故障的守卫。**实测否证（2026-09-03，别再当假设传下去）**：`actionlint` 抓不到 #1175 这一类——把**修复前**的 workflow 原样喂给它同样 rc=0 零 findings，因为它只校验 `${{ }}` 表达式，而那条 bug 是 bash 算术里的**裸字面量**（`matrix.total`），根本不在表达式位置；同一次对照实验里它也没报 `if; then` 这种明显的 shell 语法错误，只报了 `${{ matrix.nope }}` 这类未知属性。它对**现存** workflow 也是零 findings ⇒ 加进去没有噪声成本、值得加，但定位是**表达式层拼写守卫**，**不是**本类故障的守卫；本类故障只能由 step 内的数量断言守住（`ceil((N - S) / T)` + 空分片即错误）。
- **发布事实（2026-09-03，已 publish = repo Latest）**：tag `v0.17.1` → master `292208fd`，tag 管道全绿，其中三道与本节直接相关的门都过了：`Verify release consistency (tag == package.json == CHANGELOG)` · `Smoke linux-amd64 release binary` · **`Reject internal information in public release notes`**（仓库自带的 release body 卫生门）。draft notes 人工审阅：12 资产（5 平台 server + 6 migrate 二进制 + `checksums.txt` + `install.sh`）、body 与 `CHANGELOG.md` 的 `## [v0.17.1]` 节**逐字节相同**（10070 字节 / 5092 字符，差 1 字节是尾换行）、私有路径 / 主机事实 / 内部工具名扫描零命中 → `gh release edit --draft=false`。
- **发布产物活体复验（验的是产物本身，不是本地重建的近似物）**：下载 `metapi-linux-amd64` → sha256 与 `checksums.txt` 一致、`--version` 报 `v0.17.1`、`install.sh` 下载基址仍是 `releases/latest/download`；换进真实上游测试床后带鉴权核对 `GET /api/about` = `version v0.17.1` / `commit 292208fd…`（正是 tag 指向的提交），随后 new-api 链 **13 PASS / 1 WARN / 0 FAIL** · sub2api 链 **11 PASS / 2 WARN / 0 FAIL** · axonhub **PASS** · gpt-load **PASS=13 FAIL=0** · F5 双语视觉 **13/13** · 五服务全 RUNNING。WARN 全是环境性的（无上游渠道 ⇒ 路由回落 `PROXY_MODEL`）。
- **#1178 的活体前后对照（同一测试床、同一脚本、只换二进制）**：v0.17.0 产物的启动日志里有 **1 条** `embedded web/dist subtree not readable, serving disabled dir=assets`；v0.17.1 产物 **0 条**。同时 `/static/js/index.*.js` 仍 **200 / `text/javascript` / 141242 B** ⇒ 删掉那个不可达挂载没有伤到任何真实资产路径；`/assets/*` 现在落回 SPA fallback（`200 text/html`），而构建产物里本来就无人引用该前缀。
