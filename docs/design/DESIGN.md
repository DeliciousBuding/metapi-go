...
| Type | `--font-sans` Public Sans Variable + CJK sans fallbacks (Noto Sans SC/TC/JP/KR, PingFang, Microsoft YaHei) · `--font-serif` Lora Variable + CJK serif fallbacks · `--font-mono` Cascadia/SFMono/Consolas stack · `--font-body` active axis | All fonts bundled locally or resolved from the OS stack; no external font CDN |
