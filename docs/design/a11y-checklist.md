...
11. ~~**Automated axe a11y CI** gate.~~ **Done** — the `a11y` job in `.github/workflows/main.yml` serves the embedded SPA via the Go server (fresh sqlite runtime DB, `dev-admin-token-123`) and runs `a11y:scan` (15 admin routes) against the shipped bundle; gated by `docker-build` alongside the tests. Verified green in CI 2026-08-12 (commits `3c46318`, `a2c1364`).
