...
// TestCascadeIsolation_MultiChannel5xxStorm_ChannelScopedExclude is an HTTP-path
// load-proof for cascade isolation.
...
// This strengthens automated evidence. Inventory remains
// **partial** until production/live multi-channel e2e (do not flip present).
func TestCascadeIsolation_MultiChannel5xxStorm_ChannelScopedExclude(t *testing.T) {
	const budget = 3
	...
	for _, id := range mockR.selectedIDs {
		if seen[id] {
			t.Fatalf(": channel %d selected twice under storm: %v", id, mockR.selectedIDs)
		}
		seen[id] = true
	...
	t.Logf(" HTTP load-proof: selected=%v excludes=%v status=%d — inventory remains partial (no prod e2e)",
		mockR.selectedIDs, mockR.excludeSnapshots, rec.Code)
}

// TestCascadeIsolation_5xxThenHealthySiblingSucceeds proves channel-scoped failover
// recovers on a healthy sibling without re-hitting the failed channel.
func TestCascadeIsolation_5xxThenHealthySiblingSucceeds(t *testing.T) {
	...
	t.Logf(" HTTP recover: selected=%v excludes=%v", mockR.selectedIDs, mockR.excludeSnapshots)
}
