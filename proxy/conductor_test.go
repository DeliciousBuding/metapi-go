...
// TestConductor_CascadeLoadProof_MixedStormChannelScopedExclude is the M50
...
// 5xx is the pure failover path. See TestConductor_CascadeLoadProof_429SameChannelThenBudget.

// Residual honesty only — does not flip inventory partial → present.
func TestConductor_CascadeLoadProof_MixedStormChannelScopedExclude(t *testing.T) {
	const budget = 5
		...
		SelectNextChannel: func(ctx context.Context, requestedModel string, excludeChannelIDs []int64, policy routing.DownstreamRoutingPolicy) (*routing.SelectedChannel, error) {
			...
			for _, id := range excludeChannelIDs {
				if id < 1 || id > int64(channelCount) {
					t.Fatalf(" load-proof: exclude contains non-channel id %d", id)
				}
			}
	...
	for _, id := range seen {
		if seenSet[id] {
			t.Fatalf(" load-proof: channel %d attempted twice under storm: %v", id, seen)
		}
		seenSet[id] = true
	}
	...
	t.Log(" load-proof honesty: multi-channel 5xx storm stays channel-scoped and budget-bound; inventory remains partial")
}

// TestConductor_CascadeLoadProof_429SameChannelThenBudget documents that bare 429
...
// This is intentional policy — not cascade poison — and keeps partial
// (production multi-channel 429 storms still need e2e evidence).
func TestConductor_CascadeLoadProof_429SameChannelThenBudget(t *testing.T) {
	ch1 := conductorChannel(1)
	nextCalls := 0
	attemptCalls := 0
	...
	t.Log(" honesty: 429 burns same-channel budget by design; not a sibling cascade")
}
