...
describe('resolveConversationFileCapability', () => {
  ...
  it('builds the accept list from the supported file kinds', () => {
    expect(
      buildConversationFileAccept(resolveConversationFileCapability('claude'))
    ).toBe('.pdf,.txt,.md,.markdown,.json,image/*')
    expect(
      buildConversationFileAccept(resolveConversationFileCapability('gemini'))
    ).toBe('.pdf,.txt,.md,.markdown,.json,image/*,audio/*')
  })

  it('describes the actual per-kind transport behavior', () => {
    expect(
      buildConversationFileHint(resolveConversationFileCapability('claude'))
    ).toBe(
      '支持 PDF / TXT / Markdown / JSON / 图片；文档会以内联数据注入，图片会按图片部件发送。'
    )
    expect(
      buildConversationFileHint(resolveConversationFileCapability('gemini'))
    ).toBe(
  ...
  it('rejects unsupported file kinds before send', () => {
    ...
    expect(
      isConversationUploadedFileSupported(claudeCapability, {
        filename: 'paper.pdf',
