...
export type RouteModelCandidatesByModelName = Record<
  string,
  IndexedRouteModelCandidate[]
>
...
export function buildRouteModelCandidatesIndex(
  ...
  for (const route of routes || []) {
    ...
    for (const [modelName, candidates] of Object.entries(
      modelCandidates || {}
    )) {
    ...
    for (const candidate of routeCandidates) {
      if (!accountMap.has(candidate.accountId)) {
        accountMap.set(
          candidate.accountId,
          `${candidate.username || `account-${candidate.accountId}`} @ ${candidate.siteName}`
        )
      }
    ...
    const accountOptions = [...accountMap.entries()].map(([id, label]) => ({
      id,
      label,
    }))
    index[route.id] = {
