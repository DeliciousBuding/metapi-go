...
export function shouldHydrateMarketplaceMetadata<
  T extends MarketplaceModelMetadataShape,
>(models: T[]): boolean {
  return models.some((model) => !hasMarketplaceMetadata(model))
}

export function mergeMarketplaceMetadata<T extends MarketplaceModelLike>(
  baseModels: T[],
  detailedModels: T[]
): T[] {
  ...
  return baseModels.map((model) => {
    ...
    return {
      ...
      supportedEndpointTypes: Array.isArray(detail.supportedEndpointTypes)
        ? detail.supportedEndpointTypes
        : [],
      pricingSources: Array.isArray(detail.pricingSources)
        ? detail.pricingSources
        : [],
    }
  })
}
