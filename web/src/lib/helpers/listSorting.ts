...
export function sortItemsForDisplay<T extends SortableBase>(
  ...
  const customComparator = (a: T, b: T) => {
    ...
    const aOrder = Number.isFinite(a.sortOrder as number)
      ? Number(a.sortOrder)
      : Number.MAX_SAFE_INTEGER
    const bOrder = Number.isFinite(b.sortOrder as number)
      ? Number(b.sortOrder)
      : Number.MAX_SAFE_INTEGER
...
export function buildCustomReorderUpdates<T extends SortableBase>(
  ...
  next.forEach((item, idx) => {
    const prev = Number.isFinite(item.sortOrder as number)
