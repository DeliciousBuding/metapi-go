...
describe('downstreamKeyProxyUrl', () => {
  it('trims and treats empty as inherit', () => {
    ...
    expect(normalizeDownstreamKeyProxyUrl('  http://127.0.0.1:7890  ')).toBe(
      'http://127.0.0.1:7890'
    )

    expect(parseDownstreamKeyProxyUrl('')).toEqual({ valid: true, value: null })
    expect(parseDownstreamKeyProxyUrl('   ')).toEqual({
      valid: true,
      value: null,
    })
    expect(parseDownstreamKeyProxyUrl(null)).toEqual({
      valid: true,
      value: null,
    })
  ...
  it('accepts http/https/socks schemes and rejects others', () => {
    ...
    expect(
      parseDownstreamKeyProxyUrl('socks5h://user:pass@10.0.0.1:1080')
    ).toEqual({
      valid: true,
  ...
  it('formats compact indicator and redacts userinfo passwords', () => {
    ...
    expect(formatDownstreamKeyProxyIndicator('http://127.0.0.1:7890')).toBe(
      '127.0.0.1:7890'
    )
    expect(
      formatDownstreamKeyProxyIndicator('socks5://proxy.example.com:1080')
    ).toBe('proxy.example.com:1080')
    expect(
      formatDownstreamKeyProxyIndicator(
        'http://user:s3cret@proxy.example.com:8080'
      )
    ).toBe('proxy.example.com:8080')
    expect(
      formatDownstreamKeyProxyIndicator('socks5h://alice:hunter2@10.0.0.9:1080')
    ).toBe('10.0.0.9:1080')
    // Unparseable values still must not leak credentials.
    expect(
      formatDownstreamKeyProxyIndicator('not-a-url://user:pass@host')
    ).toBe('host')
    expect(
      formatDownstreamKeyProxyIndicator('not-a-url://user:pass@host')
    ).not.toContain('pass')
