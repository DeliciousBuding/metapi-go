...
function renderInlineMarkdown(text: string): string {
  ...
  html = html.replaceAll(
    ...
    (_, alt, src, title) => {
      ...
      return pushToken(
        `<img src="${escapeHtml(safeSrc)}" alt="${escapeHtml(String(alt || ''))}"${titleAttr}>`
      )
    }
  )
  html = html.replaceAll(
    ...
    (_, label, href, title) => {
      ...
      const titleAttr = title ? ` title="${escapeHtml(String(title))}"` : ''
      return pushToken(
        `<a href="${escapeHtml(safeHref)}" target="_blank" rel="noopener noreferrer"${titleAttr}>${escapeHtml(String(label || ''))}</a>`
      )
    }
  )
...
function consumeMarkdownList(
  ...
  while (
    index < lines.length &&
    isMarkdownListLine(lines[index] || '', ordered)
  ) {
...
function consumeMarkdownFence(lines: string[], startIndex: number) {
  ...
  while (
    index < lines.length &&
    !String(lines[index] || '').startsWith(marker)
  ) {
...
function sanitizeNode(node: Node): string {
  ...
  const childrenHtml = [...element.childNodes]
  ...
  if (tag === 'a') {
    ...
    const colspan = Number.parseInt(
      String(element.getAttribute('colspan') || ''),
      10
    )
