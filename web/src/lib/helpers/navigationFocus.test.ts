...
describe('navigationFocus helpers', () => {
  it('builds focus paths for site and account', () => {
    ...
    expect(buildAccountFocusPath(34, { openRebind: true })).toBe(
      '/accounts?focusAccountId=34&openRebind=1'
    )
  ...
  it('parses focus params from query string', () => {
    ...
    expect(
      readFocusAccountIntent('?focusAccountId=22&openRebind=true')
    ).toEqual({
      accountId: 22,
      openRebind: true,
    })
  ...
  it('clears focus params but keeps other params', () => {
    ...
    expect(clearFocusParams('?focusAnnouncementId=44&type=site_notice')).toBe(
      '?type=site_notice'
    )
    expect(clearFocusParams('?focusAccountId=2&openRebind=1&type=token')).toBe(
      '?type=token'
    )
  ...
  it('builds event navigation path by related entity', () => {
    ...
    expect(buildEventNavigationPath({ type: 'site_notice' })).toBe(
      '/site-announcements'
    )
