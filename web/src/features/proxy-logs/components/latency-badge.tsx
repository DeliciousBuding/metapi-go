...
const LATENCY_TIERS = {
  fast: { thresholdMs: 500, className: 'bg-success/10 text-success border-success/30', dotClassName: 'bg-success' },
  slow: { thresholdMs: 2000, className: 'bg-warning/10 text-warning border-warning/30', dotClassName: 'bg-warning' },
  unhealthy: { thresholdMs: Number.POSITIVE_INFINITY, className: 'bg-destructive/10 text-destructive border-destructive/30', dotClassName: 'bg-destructive' },
} as const
...
export function LatencyBadge({ latencyMs, firstByteLatencyMs, className, showDot = false }: LatencyBadgeProps) {
  ...
  const title =
    firstByteLatencyMs !== null && firstByteLatencyMs !== undefined && firstByteLatencyMs >= 0
      ? t('proxyLogs.latency.totalAndFirstByte', { label, firstByte: formatLatency(firstByteLatencyMs) })
