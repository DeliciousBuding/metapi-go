  ...
  return (
    <div className='space-y-2'>
      <h3 className='text-sm font-medium'>
        {t('settings.operations.database.migration.status.resultTitle')}
      </h3>
      <dl className='text-muted-foreground grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs'>
        <dt>{t('settings.operations.database.migration.status.target')}</dt>
        <dd className='font-mono break-all'>{result.connection}</dd>
        <dt>{t('settings.operations.database.migration.status.version')}</dt>
        <dd className='font-mono'>{result.version}</dd>
        {finishedAt ? (
          <>
            <dt>
              {t('settings.operations.database.migration.status.finishedAt')}
            </dt>
      ...
      {tableEntries.length > 0 ? (
        <div className='space-y-1'>
          <p className='text-muted-foreground text-xs'>
            {t('settings.operations.database.migration.status.rowsTitle')} ·{' '}
            {t('settings.operations.database.migration.status.rowsTotal', {
              count: totalRows,
            })}
          </p>
...
export function DatabaseMigrationSection() {
  ...
  useEffect(() => {
    ...
    if (task.status === 'succeeded') {
      toast.success(t('settings.operations.database.migration.toast.completed'))
    } else {
      toast.error(
        task.error ?? t('settings.operations.database.migration.toast.failed')
      )
    }
  }, [task, t])
  ...
  const startMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      toast.info(t('settings.operations.database.migration.toast.started'))
    },
    onError: (error) => {
      setConfirmOpen(false)
  ...
  function handleTestConnection() {
    const connection = requireConnection(
      'settings.operations.database.migration.schema.testConnectionRequired'
    )
  ...
  function handleStartMigration() {
    const connection = requireConnection(
      'settings.operations.database.migration.schema.connectionRequired'
    )
    if (!connection) return
    setConfirmOpen(true)
  }
  ...
  return (
    ...
    >
        ...
        {activeConfig ? (
          <div className='bg-muted/25 rounded-lg border p-3'>
            <p className='text-muted-foreground text-xs font-medium'>
              {t('settings.operations.database.migration.sourceLabel')}
            </p>
          ...
          <form className='space-y-4'>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {t('settings.operations.database.migration.fields.dialect')}
                  </FormLabel>
                  <Select
                    value={field.value}
                    disabled={busy}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {t(
                      'settings.operations.database.migration.fields.connectionString'
                    )}
                  </FormLabel>
                  <FormControl>
                    <Input
                  ...
                  <FormDescription>
                    {t(
                      'settings.operations.database.migration.fields.connectionStringHint'
                    )}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
                  ...
                  <FormItem className='flex flex-row items-start gap-3 rounded-lg border p-3'>
                    ...
                    <div className='space-y-1'>
                      <FormLabel className='cursor-pointer'>
                        {t('settings.operations.database.migration.fields.ssl')}
                      </FormLabel>
                      <FormDescription>
                        {t(
                          'settings.operations.database.migration.fields.sslHint'
                        )}
                      </FormDescription>
                    </div>
                  </FormItem>
              ...
              render={({ field }) => (
                <FormItem className='flex flex-row items-start gap-3 rounded-lg border p-3'>
                  ...
                  <div className='space-y-1'>
                    <FormLabel className='cursor-pointer'>
                      {t(
                        'settings.operations.database.migration.fields.overwrite'
                      )}
                    </FormLabel>
                    <FormDescription>
                      {t(
                        'settings.operations.database.migration.fields.overwriteHint'
                      )}
                    </FormDescription>
                  </div>
                </FormItem>
              ...
              <div className='flex flex-wrap gap-2'>
                ...
                >
                  {testMutation.isPending
                    ...
                    : t(
                        'settings.operations.database.migration.testConnection'
                      )}
                </Button>
                <Button
                  type='button'
                ...
                >
                  {startMutation.isPending || taskIsActive
                    ? t('settings.operations.database.migration.migrating')
                    : t(
                        'settings.operations.database.migration.startMigration'
                      )}
                </Button>
              </div>
            </div>
          ...
          <div className='space-y-3 rounded-lg border p-3'>
            <div className='flex flex-wrap items-center gap-2'>
              <Badge variant={taskStatusBadgeVariant[task.status]}>
                {t(
                  `settings.operations.database.migration.status.${task.status}`
                )}
              </Badge>
            ...
            {task.logs && task.logs.length > 0 ? (
              <div className='space-y-1'>
                <p className='text-muted-foreground text-xs'>
                  {t('settings.operations.database.migration.status.logsTitle')}
                </p>
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={t('settings.operations.database.migration.confirm.title')}
        description={t(
          'settings.operations.database.migration.confirm.description'
        )}
        confirmLabel={t(
          'settings.operations.database.migration.confirm.confirmLabel'
        )}
