const CONVERSATION_DOCUMENT_ACCEPT_PARTS = [
  '.pdf',
  '.txt',
  '.md',
  '.markdown',
...
export function detectConversationFileKind(file) {
  ...
  if (DOCUMENT_EXTENSIONS.some((extension) => filename.endsWith(extension)))
    return 'document'
  if (IMAGE_EXTENSIONS.some((extension) => filename.endsWith(extension)))
    return 'image'
  if (AUDIO_EXTENSIONS.some((extension) => filename.endsWith(extension)))
    return 'audio'
  return 'unknown'
}
