...
export type DownstreamKeyEditorForm = {
  ...
  maxTpm: string;
  /** Optional per-key weight scalar; empty = 1.0. */
  keyWeight: string;
  expiresAt: string;
  enabled: boolean;
  /** Per-key egress proxy; empty = inherit site/account/system. */
  proxyUrl: string;
  /** Per-key IP allowlist; newline/comma-separated CIDR/exact; empty = unrestricted. */
  ipAllowlist: string;
  /** Per-key IP blocklist; blocklist wins over allowlist; empty = no denies. */
  ipBlocklist: string;
  ...
  return (
    ...
    >
      ...
      <div className="downstream-key-modal-grid" style={{ gridTemplateColumns: '1fr' }}>
        ...
        <div className="downstream-key-modal-field downstream-key-modal-field-full">
          <div className="downstream-key-modal-help">
            RPM / TPM 为每分钟软准入窗口（）。留空或 0 表示不限；正整数生效。
          </div>
        </div>
