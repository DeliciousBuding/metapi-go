...
const { apiMock } = vi.hoisted(() => ({
  apiMock: {
    ...
    getBalanceIncomeOutcome: vi.fn().mockResolvedValue({ days: 30, points: [], summary: { totalIncome: 0, totalOutcome: 0, net: 0, accounts: 0 } }),
...
describe('Dashboard hook order', () => {
  ...
  it('keeps hook order stable when switching from loading to loaded render', async () => {
    ...
    const errorOutput = consoleErrorSpy.mock.calls
      .map((args: unknown[]) => args.map((item: unknown) => String(item)).join(' '))
      .join('\n');
