...
// first-run sidebar progressive disclosure — with
...
function AppShell() {
  ...
  // table density (comfortable default / compact via data-density).
  const [density, setDensity] = useState<DensityMode>(() =>
    resolveInitialDensity((k) => localStorage.getItem(k)),
  );
  // true = no sites yet (onboarding); null = not loaded (no folding).
  ...
  const resolvedTheme: 'light' | 'dark' = themeMode === 'system'
    ...
    : themeMode;
  // fold non-core nav when first-run and not expanded.
  ...
  // visual acceptance. Still gated by DEV or localStorage flag.
  ...
  // apply the accent preset (blue default needs no attribute, mirroring
  ...
  // apply table density (comfortable = default, no attribute). Same
  ...
  // detect first-run (no sites) for sidebar progressive disclosure.
          ...
          <div ref={themeMenuRef} style={{ position: 'relative' }}>
            ...
            {themeMenuPresence.shouldRender && (
              <div className={`user-dropdown ${themeMenuPresence.isVisible ? '' : 'is-closing'}`.trim()} style={{ right: 0, left: 'auto', minWidth: 168 }}>
                ...
                {/* accent preset picker (blue default / indigo / teal) */}
                <div
                  className="user-dropdown-item"
                ...
                {/* table density toggle (comfortable 8px / compact 6px) */}
                <div
                  className="user-dropdown-item"
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'default', gap: 8 }}
                >
      ...
      <div className="app-layout">
        {isMobile ? (
          ...
          <aside className={`sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
            ...
            >
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true" style={{ transform: sidebarCollapsed ? 'rotate(180deg)' : 'none', transition: 'transform 0.3s ease', flexShrink: 0 }}>
