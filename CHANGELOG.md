...
站点 API 端点编辑器、探针产品化、全路由截图回归门禁、代理全失败告警；修复备份导入的凭据安全问题（**建议所有旧版本尽快升级**）与一系列界面、主题对比度、稳定性问题。

### Security

- **备份导入凭据安全（#941）**：修复恶意备份可植入凭据外传配置的问题——导入拒绝清单已增补、下游密钥导入不再回显值。旧版本（≤v0.16.7）存在此风险，建议尽快升级。

### Added

- **apiEndpoints 编辑器（#935）**：站点表单多行端点编辑（校验与后端一致，含 SSRF 拒绝与去重）。
- **探针产品化（#939）**：站点探针 SSE 实时报告视图、手动触发与最近运行历史、模型批量探针对话框。
- **代理全失败告警（#942）**：上游全部失败时发送通知（按模型节流）。
- **docs/api.md 补齐（#940）**：全部已注册端点补全文档。

### Fixed

- **可空列同族（#943）**：路由/站点/OAuth 可空数值列指针化，单行 NULL 不再击穿路由装载。
- **主题对比度（#949）**：多处低对比度（最差 1.50:1）全部修复至 WCAG AA（≥4.5:1）。
- **UX 动线（#948）**：导入向导工具栏入口、完成步深链接入建号流程、移动端密钥页卡片降级、签到按钮加载可用性。
- **优雅停机（#946）**：等待在途调度任务有界排空，减少启动抖动。
- **界面文本本地化收官（#938）**：用户可见文本全部本地化。

### Performance

- **代理热路径（#944）**：无映射请求零拷贝短路 + 单遍流式标志重写。
- **路由热路径（#947）**：路由 TTL 缓存接线 + 正则编译缓存 + bucket 惰性扫描。
- **proxy-logs（#945）**：统计聚合单遍化 + 短期缓存 + 覆盖索引。
...
- **上游路径穿越（#922）**：代理转发前拒绝含 `..` 的路径段，防止越出站点 API 前缀。
- **NULL 余额 500（#931）**：迁移/未刷新账号不再让站点列表整页报错。
- **空库快照序列化（#928）**：空库下账号/站点快照返回空数组，消除前端静默重试。
- **3 处假成功（#930）**：签到触发忽略失败结果、trigger-all 部分失败、代理测试误报可用——均改为呈现真实结果。
- **CSV 注入（#925）**：导出面公式前缀单元格转义。
- **accounts 快照竞态 + 命中区 + 焦点序（#924）**：新建站点后添加账号入口延迟可点、侧栏命中区、登录页 Tab 顺序。
- **a11y（#920）**：表单错误 role=alert、对比度探针导航竞态。
- **安全硬化（#917）**：公共路由路径段拒绝、SSRF 加固；依赖漏洞扫描零可达漏洞。
