...
Wave 5 + Wave 6「开发 + 深度审计波」：14 条并行线——功能收口（#861/#558/#926）、测试矩阵扩展（真实上游 2/16→4/16、运行时矩阵 1/5→4/5）、截图视觉回归管道落地、6 维深审计（架构/动线/视觉/性能/安全，22 项坐实发现全修复）。

### Security

- **备份导入凭据植入（P0，#941）**：恶意备份可植入 `backup_webdav_config_v1` 自动同步配置周期性外传全库凭据——导入拒绝清单增补 4 个运行时本地键（两条导入路径共用）；下游密钥导入保留但响应+审计事件显式列出新增条目（不回显值）；修复 PG 上审计事件静默失败（boolean 列字面量）。

### Added

- **apiEndpoints 编辑器（#935，关 #861）**：站点表单多行端点编辑（校验镜像后端：normalize + SSRF 拒绝 + 去重），未触碰保留语义不变。
- **探针产品化（#939，关 #558）**：站点探针 SSE 实时报告视图、调度器手动触发 + 最近运行历史、模型批量探针对话框（失败/未知态诚实呈现）。
- **截图测试双管道（#951）**：CI 全路由截图证据管道（110 张 × light/dark × 2xDPR，artifact 存档）+ 4 关键页 golden diff 门禁（playwright toHaveScreenshot，零新依赖）。
- **测试矩阵扩展（#937）**：test-e2e 新增 sub2api + cliproxyapi 真实容器链（真实登录/令牌/转发验证）；新 `runtime-smoke-matrix` job：macOS/Windows/ubuntu-arm64 原生构建 + 启动 + /ready + /v1 冒烟（运行时验证 1/5→4/5）。
- **代理全失败告警（#942）**：三个终态失败出口接线 `proxy_all_failed` 事件 + 通知，per-model 签名级节流（≥300s 冷却）。
- **对比度静态守卫（#949）**：解析两级主题 CSS（含 color-mix 级联），320 配对全量断言 ≥4.5:1。
- **docs/api.md 补齐（#940）**：74 条已注册未记账端点全部补齐（参数/响应从源码验证）。

### Fixed

- **可空列同族（#943）**：#933 残余——路由/站点/OAuth 单元 4 表可空数值列指针化 + 启动回填 + DDL 收紧 NOT NULL，单行 NULL 不再击穿路由装载。
- **审计残留 P1（#936）**：attention 项硬刷新改 SPA 深链、加载错误页级测试固化。
- **主题对比度 6 项（#949）**：最差 1.50:1（rose-garden 暗 secondary）与 1.75:1（lake-view 暗侧栏）等全部修复至 ≥4.5:1。
- **UX 动线 4 项（#948）**：导入向导工具栏入口找回、完成步深链接入建号流程、Downstream Keys 移动端卡片降级、签到按钮加载窗口可用性。
- **后端卫生（#946）**：优雅停机等待在途调度任务（5s 有界 drain）、启动风暴抖动、删除死代码端口-适配器。
- **前端卫生（#950）**：8 个死组件删除（1512 行）+ knip 豁免收窄、12 个一次性探针归档、时间格式化栈合并、分层倒挂修复。
- **后端消息英文化收官（#938，关 #926）**：用户可见中文归零（含 service 层），上游错误模式匹配保留。

### Performance

- **代理热路径（#944）**：无映射请求零拷贝短路 + 单遍流式标志重写（200KB 管线 19.4ms→1.0ms，分配 8991→1）。
- **路由热路径（#947）**：TTL 缓存接线（既有死代码激活）+ 正则编译缓存 + bucket 惰性扫描（每请求 783µs/94KB → 28.8µs/320B 命中）；附带修复既有数据竞争。
- **proxy-logs（#945）**：summary 单遍化 + 10s singleflight 缓存 + 5 列 covering 索引（500k 行实测 1.2-2.4s→翻页零聚合）。
