...
func TestApplyPostgresApplicationNameURL(t *testing.T) {
	got := applyPostgresApplicationName("postgres://u:p@h/db?sslmode=require", "metapi-app")
	if !strings.Contains(got, "application_name") {
		t.Fatalf("expected application_name inject, got %q", got)
	}
	keep := applyPostgresApplicationName("postgres://u:p@h/db?application_name=custom", "metapi-app")
