...
func loadRedirectCandidates(ctx context.Context, db *sqlx.DB) ([]string, error) {
	...
	if err := db.SelectContext(ctx, &routes, db.Rebind(`SELECT model_pattern FROM token_routes WHERE enabled = ?`), true); err != nil {
		return nil, err
	}
...
// ReloadRedirectRegistry reloads the in-process redirect registry from
