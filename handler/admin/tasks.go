...
// StartBackgroundTask dual-writes to `admin_background_tasks` when a
...
// when the handler DB is set. Cold DB fallback enables cross-instance visibility.
...
// available. Memory is the fast path; DB is the cold cross-process fallback.
...
func StartBackgroundTask(opts BackgroundTaskStartOptions, runner func() (any, error)) (task *BackgroundTask, reused bool) {
	...
	// Dual-write to DB best-effort. Memory is the primary layer.
...
func listBackgroundTasks(db *sqlx.DB, limit int) []BackgroundTask {
	...
	// Cold DB fallback: merge rows from other processes not in memory.
...
// ---- Durable admin_background_tasks helpers ----
...
// still touch package globals (e.g. globalAccountsCache.clear).
