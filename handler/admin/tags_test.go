...
// ---- I1 (all-api-hub borrow): accounts/sites tag system ----
...
func TestTags_UpdateAndAggregate(t *testing.T) {
	...
	resp = doPutJSON(t, r, "/api/sites/"+itoa(siteID)+"/tags", map[string]any{"tags": []string{"prod", "hk3"}})
	if resp.Code != 200 {
		t.Fatalf("site tags returned %d: %s", resp.Code, resp.Body.String())
	}

	// Aggregate index: prod 2 (1 account + 1 site), priority 1, hk3 1.
	...
	if byName["priority"] == nil || byName["hk3"] == nil {
