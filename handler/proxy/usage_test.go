...
func TestParseUsageFromBodyAnthropic(t *testing.T) {
	...
	// and must be included in prompt/total accounting (upstream).
...
// Claude/Anthropic stream merge must retain early
...
// This locks residual merge behavior — not a claim of perfect multi-instance billing.
func TestParseUsageFromSSEEvents_MergeAccount(t *testing.T) {
	events := []SseEvent{
...
// SSE streams that never emit usage must stay Found=false
...
// OpenAI chat stream end usage (include_usage style)
...
// Same merge contracts via the incremental SSE analyzer used on the live proxy path.
func TestIncrementalSseAnalyzer_MergeAccount(t *testing.T) {
...
// Gemini streamGenerateContent-style SSE emits
...
// Empty usage object / zero-only usage must not
...
// residual: media endpoints sometimes only emit input_tokens_details /
