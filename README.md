...
| **路由与容错**      | 模型自动发现；建一条路由，通道按账号模型可用性自动绑定（「自动重建」只重组已有路由的通道，不新建路由）；按成本/余额/使用权重分配多通道；失败通道自动冷却并重试下一通道；运行时熔断 + half-open 探测 |
...
在界面里添加至少一个上游站点与账号，再为要暴露的模型**建一条路由**（通道会按账号模型可用性自动绑定；「自动重建」只重组已有路由的通道，**不会新建路由**），然后像调用 OpenAI 一样调用 Metapi：
